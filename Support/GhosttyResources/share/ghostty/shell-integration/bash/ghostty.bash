...
function __ghostty_precmd() {
  ...
  if test "$_ghostty_executing" != "0"; then
    ...
    if [[ "$IGHOSTTY_DISABLE_PROMPT_MARKS" != "1" ]]; then
    ...
    if [[ "$IGHOSTTY_DISABLE_PROMPT_MARKS" != "1" && "$PS1" == *"\n"* ]]; then
      PS1="${PS1//\\n/\\n$'\\[\\e]133;P;k=s\\a\\]'}"
    fi
  ...
  if [[ "$IGHOSTTY_DISABLE_PROMPT_MARKS" != "1" ]] && test "$_ghostty_executing" != ""; then
  ...
  if [[ "$IGHOSTTY_DISABLE_PROMPT_MARKS" != "1" ]]; then
    ...
    if [[ -n "${BLE_VERSION-}" ]]; then
      ...
      # iGhostty: avoid redraw=last/cl=line because native tab/resize
      # transitions can clear the active prompt row without a shell redraw.
